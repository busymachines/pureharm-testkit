...
abstract class PureharmTest extends FunSuite with PureharmAssertions with PureharmTestRuntimeLazyConversions {
  ...
  /** The pattern is rather straightforward:
    *
    * {{{
    * final class MyTest extends PureharmTest {
    *   private val myComplexTestFixture = ResourceFixture {testOptions =>
    *     Resource.pure("maybe use testOptions to create a database name or something")
    *   }
    *
    *   myComplexTestFixuture.test("testing w/ setup and teardown") { acquiredResource =>
    *     //write test
    *     //at the end of the test, the resource gets cleaned up as per semantics of Resource.
    *   }
    * }
    * }}}
    */
