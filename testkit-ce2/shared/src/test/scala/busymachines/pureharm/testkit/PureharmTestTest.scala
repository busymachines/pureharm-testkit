...
final class PureharmTestTest extends PureharmTest {
  ...
  test("throw exception in body".fail) {
    throw new RuntimeException("This is an intentional fail, too see if munit integration in these cases works")
  }

  test("invalid exception body".fail) {
    423
  }
  ...
  failedResource.test("failed init".fail)(_ => testLogger.info("never gonna used a failed resource"))
}
